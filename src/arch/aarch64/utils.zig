...
pub fn readU32(value: []const u8) u32 {
    return std.mem.bigToNative(u32, @as(*const u32, @ptrCast(@alignCast(@alignOf(u32), value.ptr))).*);
}
...
pub fn firstReg(address_cells: u32, value: []const u8) !u64 {
    ...
    var big_endian_cells: []const u32 = @as([*]const u32, @ptrCast(@alignCast(@alignOf(u32), value.ptr)))[0 .. value.len / @sizeOf(u32)];
    if (address_cells == 1) {
