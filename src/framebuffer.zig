...
pub fn init() void {
    if (framebuffer_request.response) |framebuffer| {
        ...
        if (framebuffer_response.framebuffer_count > 0) {
            ...
            memory_model = @as(MemoryModel, @enumFromInt(@intFromEnum(framebuffer.getFramebuffers()[0].memory_model)));
...
pub fn getSlice(comptime T: type) []T {
    return @as([*]T, @ptrFromInt(address))[0 .. width * height];
}

pub fn putPixel(x: usize, y: usize, color: u32) !void {
    switch (bpp) {
        24 => getSlice(u24)[x + width * y] = @as(u24, @truncate(color)),
