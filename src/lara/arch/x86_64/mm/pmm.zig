...
const Bitmap = struct {
    ...
    pub fn set(self: Self, bit: u64) void {
        self.bits[bit / 8] |= @intCast(u8, 1) << @intCast(u3, bit % 8);
    }
    ...
    pub fn check(self: Self, bit: u64) bool {
        return (self.bits[bit / 8] & (@intCast(u8, 1) << @intCast(u3, bit % 8))) != 0;
    }
};
...
pub fn init() void {
    ...
    for (entries) |entry| {
        ...
        if (entry.length >= bitmap.size) {
            bitmap.bits = @intToPtr([*]u8, entry.base + hhdm_response.offset);

            // Initialize the bitmap to 1
            @memset(bitmap.bits, 0xff, bitmap.size);
    ...
    while (last_used_index < limit) {
        if (!bitmap.check(last_used_index)) {
            ...
            if (p == count) {
                ...
                return @intToPtr([*]u8, page * std.mem.page_size);
            }
        } else {
            last_used_index += 1;
            p = 0;
...
pub fn free(buf: ?[*]u8, count: usize) void {
    ...
    const page = @ptrToInt(buf.?) / std.mem.page_size;
