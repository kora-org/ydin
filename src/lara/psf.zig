...
pub const Psf1 = struct {
    ...
    pub fn init(font: []const u8) !Psf1 {
        ...
        ret.data = font;
        ret.header = @ptrCast(*const Header, @alignCast(@alignOf(*Header), ret.data));
        if (ret.header.magic != 0x0436)
            return error.InvalidMagic;

        ret.length = if (ret.header.flags == 1) 512 else 256;

        const last_glyph_offset = @sizeOf(Header) + @intCast(usize, ret.header.height) * ret.length;
...
pub const Psf2 = struct {
    ...
    pub fn getUnicode(self: *const Psf2, sequence: []u8) ![]const u8 {
        const index: usize = self.unicode.get(sequence) orelse if (std.ascii.isASCII(@intCast(u8, try std.unicode.utf8Decode(sequence))) and sequence.len == 1)
            sequence[0]
        else
            undefined;
